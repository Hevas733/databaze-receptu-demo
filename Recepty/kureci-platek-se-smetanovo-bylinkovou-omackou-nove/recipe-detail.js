const DATA_URL=document.body.dataset.recipeSource||'./kureci-platek-se-smetanovo-bylinkovou-omackou-nove.json';
const STORAGE_KEY=document.body.dataset.storageKey||'receptar:kureci-platek-se-smetanovo-bylinkovou-omackou-nove';
...
function render(){
  ...
  const nutrition=document.querySelector('#nutrition');if(nutrition){const n=recipe.nutritionPerServing||{};nutrition.innerHTML=[['Energie',n.energyKj==null?'—':`${n.energyKj.toLocaleString('cs-CZ')} kJ`],['Bílkoviny',n.proteinG==null?'—':`${String(n.proteinG).replace('.',',')} g`],['Tuky',n.fatG==null?'—':`${String(n.fatG).replace('.',',')} g`],['Sacharidy',n.carbohydratesG==null?'—':`${String(n.carbohydratesG).replace('.',',')} g`],['Vláknina',n.fiberG==null?'—':`${String(n.fiberG).replace('.',',')} g`]].map(([label,value])=>`<p><span>${label}</span><b>${value}</b></p>`).join('')};const review=document.querySelector('#nutritionReview');if(review)review.textContent=recipe.nutritionReview||'';const allergens=document.querySelector('#allergens');if(allergens)allergens.innerHTML=(recipe.allergens||[]).map(item=>`<p>${item.number} · ${item.name}</p>`).join('');
  document.querySelector('#instructions').textContent=recipe.instructions||'Postup zatím nebyl doplněn.';
  renderHistory();
}
...
function openEditor(){
  ...
  form.elements.preparationType.value=recipe.preparationType||'';
  form.elements.status.value=recipe.status||'active';
  form.elements.availability.value=recipe.season.availability||'';
  form.elements.recommended.value=recipe.season.recommendedDisplay||recipe.season.recommended.join('–')||'';
